...
def TRS_Trainer(args, loader: DataLoader, models, criterion, optimizer: Optimizer,
	...
	for i, (inputs, targets) in enumerate(loader):
		...
		N = inputs.shape[0] // 2
		cureps = (args.adveps - args.init_eps) * epoch / args.epochs + args.init_eps
...
def Naive_Trainer(args,x0_list , y0_list , batch_num, models, criterion, optimizer: Optimizer,epoch: int, device: torch.device, writer=None ):
    ...
    # 平移向量 self.random_noise = np.random.randint(low=self.opt.noise_lowerbound,high=self.opt.noise_upperbound,size = [32,32,3],dtype=np.uint8)
    # self.mask = np.random.choice([0,1], size=[32,32,3], p = [1-self.opt.p, self.opt.p])
	# a = [] 
	# b =[] 
	# mask = []
	# for i in range(args.num_models):
